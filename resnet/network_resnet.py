...
def resnet83(images, n_classes, is_training, weight_decay):
    ...
    for stage, (f, l, s) in enumerate(zip(n_filters, n_layers, n_strides)):
        ...
        stage += 1
        block = chr(ord('a'))
        x = bottleneck_residual_block_v2(x, prev_filter, f, stage, block, is_training, strides=s, scale=weight_decay)
        for ii in range(1, l):
            block = chr(ord(block) + 1)
            x = bottleneck_residual_block_v2(x, f, f, stage, block, is_training, strides=1, scale=weight_decay)
        prev_filter = f
